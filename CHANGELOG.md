...
### Added
- **Job-failure counter + phone alerting for invoice worker failures** — the Grafana "Needs Attention" dashboard row now surfaces job failures directly, and failures / stuck jobs / workflow backlog now push to the phone via gotify instead of only Telegram. Backed by a new `invoice_worker_failed_total` metric (labelled by failure reason and workflow type), which also closes a blind spot: malformed-input failures that occur before the email-carrying trace span previously left no signal at all — the counter is zero-seeded at startup so those failures are now visible even when they've never fired.
