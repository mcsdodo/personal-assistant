...
### Fixed
- claude-code: reprocessing with `force: true` no longer fails when the document it means to refresh was deleted. The refresh target is read from the record of what a previous job uploaded, and that record is never cleared when a document is deleted in Paperless, so the update went to a document that was gone, came back "not found", and the job spent all of its retries on a request that no retry can satisfy. Because the record still pointed at the deleted document, every later attempt failed the same way. A missing target now means there is nothing to refresh, so the job uploads a fresh document instead and says so in its own event log. Any other update failure still fails the job, because a second copy of a document that does exist is a duplicate. Both the email pipeline and the scan pipeline are covered
...
- claude-code: the document-classifier now has a rule for the accounting period of a non-monetary document, where it had none at all. An attendance sheet, a travel order or a monthly report is filed under **the period it covers**, not the date the form was filled in - a travel order for a trip on 24-25 March is March even when the form is dated 1 April. A document that covers no period at all, such as a shop's terms and conditions, returns no period. This behaviour was previously unspecified and depended entirely on the model's judgment
- claude-code: on the email path, the accounting month of a `document` is no longer taken from the issue date printed on it when the classifier reasoned no period. Such a document has no purchase date, so the month falls through to the email's arrival date instead. Two things are deliberately unchanged: bank statements, whose issue date does relate to the period they cover, and the scan pipeline, whose non-monetary documents are attendance sheets and travel orders that belong to the month printed on them rather than the month they were scanned
- claude-code: `force: true` reprocessing of an email now PATCHes the existing Paperless document even when the document carries no order number. The PATCH path was reached only through a duplicate match on the order number, so reprocessing a payslip - or, after the change above, any non-monetary document - quietly uploaded a fresh copy instead, which Paperless then discarded as identical content. The job reported success and nothing changed. The prior document id is now resolved from the job's own source reference, which is the same approach the scan pipeline already used
...
### Fixed
- email-poller: backlog gauge was counting cancelled jobs as active; jobs gauge now zero-fills emptied states (kills phantom Waiting-On-You / Backlog counts)
...
### Added
- **Job-failure counter + phone alerting for invoice worker failures** — the Grafana "Needs Attention" dashboard row now surfaces job failures directly, and failures / stuck jobs / workflow backlog now push to the phone via gotify instead of only Telegram. Backed by a new `invoice_worker_failed_total` metric (labelled by failure reason and workflow type), which surfaces all terminal failures: intake-level issues (malformed input, schema errors, missing owner, intake errors), dispatcher-level failures (unsupported workflows, worker exceptions), and timeout conditions (stale jobs, guidance timeouts). Pre-span failures that previously left no signal at all are now visible — the counter is zero-seeded at startup.

### Fixed
- **Worker metrics never reached Prometheus (latent since the task-64 worker split)** — every worker-side counter (`invoice_worker_*`, `personal_assistant_guidance_requests_total`) was constructed at module-load, *before* `initTracing()` registered the OTel MeterProvider. OTel JS's ProxyMeter does not upgrade instruments created before `setGlobalMeterProvider`, so those counters silently exported nothing for 30d+ (traces were unaffected — spans are created at call time). Added a [`tracing-bootstrap.ts`](claude-code/channels/tracing-bootstrap.ts) side-effect module imported first in [`worker.ts`](claude-code/channels/worker.ts), so init runs before any counter is built. This is what makes the new `invoice_worker_failed_total` (and the previously-dead "Top Correspondents" / guidance-request dashboard panels) actually populate.
