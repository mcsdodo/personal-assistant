...
describe("tryDecrypt", () => {
  ...
  test("with password set: calls qpdf for encrypted file", async () => {
    ...
    writeFileSync(helperPath, [
      ...
      `  expect(calls[0][0]).toBe("qpdf");`,
      `  expect(calls[0][1]).toContain("--is-encrypted");`,
      `  expect(calls[1][0]).toBe("qpdf");`,
      `  expect(calls[1][1].join(" ")).toInclude("--password=");`,
      `  expect(calls[1][1]).toContain("--decrypt");`,
      `});`,
    ].join("\n"));
...
describe("isPdfEncrypted", () => {
  ...
  test("exit code 0 from qpdf → returns true (subprocess with mocked execSync)", async () => {
    ...
    writeFileSync(helperPath, [
      ...
      ``,
      `mock.module("child_process", () => ({`,
      `  execFileSync: (_cmd: string, _args: string[], _opts?: any) => Buffer.from(""),`,
      `}));`,
      ``,
  ...
  test("exit code 2 from qpdf → returns false (subprocess with mocked execSync)", async () => {
    ...
    writeFileSync(helperPath, [
      ...
      ``,
      `mock.module("child_process", () => ({`,
      `  execFileSync: (_cmd: string, _args: string[], _opts?: any) => {`,
...
describe("tryDecryptWithPassword", () => {
  ...
  test("calls qpdf --decrypt with supplied password when file is encrypted", async () => {
    ...
    writeFileSync(helperPath, [
      ...
      `  expect(calls[0][1]).toContain("--is-encrypted");`,
      `  expect(calls[1][0]).toBe("qpdf");`,
      `  expect(calls[1][1].join(" ")).toInclude("--password=userPassword123");`,
      `  expect(calls[1][1]).toContain("--decrypt");`,
      `});`,
    ].join("\n"));
