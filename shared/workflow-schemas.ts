 ...
 *
 * Single source of truth, shared by claude-code/channels and pollers via
 * 1-line re-export barrels. Lives in shared/ (stack root) — this dir is
 * dependency-free BY DESIGN (bun:sqlite + node builtins only): Docker images
 * copy it to /shared without a node_modules, and Bun resolves npm packages
 * by walking up from the importing file, so npm imports here would break
 * every consumer. Do not add npm dependencies to shared/.
 *
