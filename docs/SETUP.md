...
Google OAuth refresh tokens are long-lived (effectively permanent for Desktop app clients). Re-auth is only needed if:
- Tokens are revoked in Google Account settings
- The credential volume is wiped
- OAuth scopes change (e.g., adding Calendar)

Practically: **once at initial setup, then rarely if ever.**
