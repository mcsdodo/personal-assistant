...
**Projected income (web UI only).** The `/pl` page estimates income for the current month and any later month in the requested year that has no invoice yet: `working_days * 8h * hourly_rate`, using the same `sk_working_days()` and `PL_RATES`-driven rate lookup as the "Days worked" column. Rendered as `(projected)` per-month rows plus two summary lines: "Projected income total" = actual income + sum of projected months, and "Projected net total" = actual net income + sum of projected months. The "Days worked" ratio only counts real (non-projected) months. Inert when neither rate source in [`webapp.py`](../checker-mcp/webapp.py)'s `_load_rates()` (env var `PL_RATES`, or a gitignored local-dev `pl-rates.json`) is configured (no rate → no projection) and never touches historical months, so it can't paper over a genuinely missing invoice from the past. `collect_pl()` and `get_pl_summary()` are unaffected — this is display-only logic in `render_pl`, same file.
