...
**Code:**
- [`checker-mcp/server.py:55-72`](../local/checker-mcp/server.py) — `match_invoices()` tool: calls `collect_month()` from engine
- [`checker-mcp/server.py:75-103`](../local/checker-mcp/server.py) — `match_invoices_range()` tool: iterates months, applies `filter_resolved_unmatched()`
...
**Code:** [`checker-mcp/server.py:123-150`](../local/checker-mcp/server.py) — `get_month_status()`: runs `collect_month()`, extracts stats + missing list.
...
**Code:** [`checker-mcp/server.py:106-120`](../local/checker-mcp/server.py) — `get_pl_summary()`: calls `collect_pl()` from engine. Engine implementation at [`match_invoices.py:841-1051`](../local/checker-mcp/match_invoices.py).
...
**Code:** [`checker-mcp/server.py:27-49`](../local/checker-mcp/server.py) — `_ClientHolder` singleton: resolves `statement_type_id`, `total_amount_field_id`, `total_amount_alt_field_id`, `invoicing_tag_id`.
