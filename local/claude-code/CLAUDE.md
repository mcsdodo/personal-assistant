...
1b. **Download PDF** (if action = download_and_upload) — get the PDF to disk:
   - `claude_download` strategy: **you must download the file yourself**. List attachments using the email MCP (gmail or outlook), identify which attachment is the invoice (look for PDF, largest file, or invoice-related filename), download it to `/workspace/downloads/`, and pass the `file_path` when creating the job. The worker will NOT attempt to download — it reads from disk only. For Gmail use `get_gmail_message_content` (attachments are inline); for Outlook use `get_attachments` + `download_attachment`.
